...
public class Quaternion {
	...
	public final static float length(final float x, final float y, final float z, final float w) {
	...
	public Quaternion setEulerAngles(float pitch, float yaw, float roll) {
		return setEulerAnglesRad(Maths.degreesToRadians(pitch), Maths.degreesToRadians(yaw), Maths.degreesToRadians(roll));
	}
	...
	public float getRoll() {
		return Maths.radiansToDegrees(getRollRad());
	}
	...
	public float getPitchRad() {
		final int pole = getGimbalPole();
		return pole == 0 ? (float) Math.asin(Maths.clamp(-1f, 1f, 2f * (w * x - z * y))) : (float) pole * Maths.PI * 0.5f;
	}

	/** Get the pitch euler angle in degrees, which is the rotation around the x axis. Requires that this quaternion is normalized.
	 * @return the rotation around the x axis in degrees (between -90 and +90) */
	public float getPitch() {
		return Maths.radiansToDegrees(getPitchRad());
	}
	...
	public float getYaw() {
		return Maths.radiansToDegrees(getYawRad());
	}

	public final static float lengthSquared(final float x, final float y, final float z, final float w) {
	...
	// TODO : this would better fit into the Vector3f class
	...
	public Matrix4f toMatrix(Matrix4f matrix) {
		final float xx = x * x;
		final float xy = x * y;
		final float xz = x * z;
		final float xw = x * w;
	...
	public Quaternion setFromAxis(final float x, final float y, final float z, final float degrees) {
		return setFromAxisRad(x, y, z, Maths.degreesToRadians(degrees));
	}
	...
	public Quaternion setFromAxisRad(final float x, final float y, final float z, final float radians) {
		float d = Maths.length(x, y, z);
		if (d == 0f) return identity();
		d = 1f / d;
		float ang = radians < 0 ? Maths.PI2 - (-radians % Maths.PI2) : radians % Maths.PI2;
		float sin = Maths.sinFloat(ang / 2);
		float cos = Maths.cosFloat(ang / 2);
		return this.set(d * x * sin, d * y * sin, d * z * sin, cos).normalize();
	}
	...
	public Quaternion setFromAxes(boolean normalizeAxes, float xx, float xy, float xz, float yx, float yy, float yz, float zx, float zy, float zz) {
		if (normalizeAxes) {
			final float lx = 1f / Maths.length(xx, xy, xz);
			final float ly = 1f / Maths.length(yx, yy, yz);
			final float lz = 1f / Maths.length(zx, zy, zz);
			xx *= lx;
			xy *= lx;
			xz *= lx;
			yx *= ly;
	...
	public Quaternion setFromCross(final Vector3f v1, final Vector3f v2) {
		final float dot = Maths.clamp(-1f, 1f, Vector3f.dot(v1, v2));
		final float angle = (float) Math.acos(dot);
		return setFromAxisRad(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x, angle);
	}

	/** Spherical linear interpolation between this quaternion and the other quaternion, based on the value f in the range
	 * [0,1].
	 * @param end the end quaternion
	 * @param f alpha in the range [0,1]
	 * @return this quaternion for chaining */
	public Quaternion slerp(Quaternion end, float f) {
		...
		if ((1 - absDot) > 0.1) {// Get the angle between the 2 quaternions,
			...
			// it's sine value
			scale0 = ((float) Math.sin((1f - f) * angle) * invSinTheta);
			scale1 = ((float) Math.sin((f * angle)) * invSinTheta);
		}
	...
	public final static float dot(final float x1, final float y1, final float z1, final float w1, final float x2, final float y2, final float z2,
			final float w2) {
	...
	public float getAxisAngle(Vector3f axis) {
		return Maths.radiansToDegrees(getAxisAngleRad(axis));
	}
	 ...
	 * {@link #getAxisAngleRad(Vector3f)} to get both the axis and the angle of this rotation. Use
	 * {@link #getAngleAroundRad(Vector3f)} to get the angle around a specific axis.
	 * @return the angle in radians of the rotation */
	public float getAngleRad() {
		return (float) (2.0 * Math.acos((this.w > 1) ? (this.w / length()) : this.w));
	}

	/** Get the angle in degrees of the rotation this quaternion represents. Use {@link #getAxisAngle(Vector3f)} to get both the axis
	 * and the angle of this rotation. Use {@link #getAngleAround(Vector3f)} to get the angle around a specific axis.
	 * @return the angle in degrees of the rotation */
	public float getAngle() {
		return Maths.radiansToDegrees(getAngleRad());
	}
	...
	public float getAngleAroundRad(final Vector3f axis) {
		final float d = Vector3f.dot(new Vector3f(x, y, z), axis);
		final float l2 = Quaternion.lengthSquared(axis.y * d, axis.y * d, axis.z * d, this.w);
		return (l2 == 0.0f) ? 0f : (float) (2.0 * Math.acos(Maths.clamp(-1f, 1f, (float) ((d < 0 ? -this.w : this.w) / Math.sqrt(l2)))));
	}
}
