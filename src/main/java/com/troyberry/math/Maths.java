...
public class Maths {
	...
	/**
	 * Interpolates using a cosine wave between the values a and b using the factor f<br>
	 * The factor f will be clamped between 0.0 and 1.0 before the calculation is carried out, 
	 * therefore the result will always lie somewhere between and a b inclusive.
	 * @param a The first number
	 * @param b The second number
	 * @param f The factor to interpolate between a and b (automaticalld clamped between 0.0 and 1.0)
	 * @return The interpolated output
	 */
	...
	public static float getDistanceBetweenPoints(Vector3f point1, Vector3f point2) {
		return (float) Math
				.sqrt(Math.pow(point1.x - point2.x, 2) + Math.pow(point1.y - point2.y, 2) + Math.pow(point1.z - point2.z, 2));
	}
	...
	public static double approximateDistanceBetweenPoints(Vector3f point1, Vector3f point2) {
		return Math.abs(Math.abs(point1.x) - Math.abs(point2.x)) + Math.abs(Math.abs(point1.y) - Math.abs(point2.y))
				+ +Math.abs(Math.abs(point1.z) - Math.abs(point2.z));
	}
	...
	public static double evaluate(String expression, final boolean degrees) throws NumberFormatException {
		...
		return new Object() {
			...
			double parse() {
				...
				if (pos < e.length()) { throw new NumberFormatException("Unexpected: " + (char) ch); }
				return x;
			}
			...
			double parseFactor() {
				...
				if (eat('(')) { // parentheses
					...
					else if (func.equalsIgnoreCase("sin")) x = Math.sin(degrees ? Math.toRadians(x) : x);
					else if (func.equalsIgnoreCase("cos")) x = Math.cos(degrees ? Math.toRadians(x) : x);
					else if (func.equalsIgnoreCase("tan")) x = Math.tan(degrees ? Math.toRadians(x) : x);
					else if (func.equalsIgnoreCase("asin")) x = Math.asin(degrees ? Math.toRadians(x) : x);
					else if (func.equalsIgnoreCase("acos")) x = Math.acos(degrees ? Math.toRadians(x) : x);
					else if (func.equalsIgnoreCase("atan")) x = Math.atan(degrees ? Math.toRadians(x) : x);
	 ...
	 * To parse with degrees, use <code>Maths.evaulate(expression, true);</code>
	 * Examples:<br>
	 ...
	 * @param radius The radius of the sphere that points are on
	...
	/**This method converts a float to a double preventing sign flipping or other other weird issues if the double is out of the representable range for a float.<br>
	...
	public static boolean rayTriangleIntersect(Vector3d o, Vector3d d, Vector3d v1, Vector3d v2, Vector3d v3) {
		...
		if (a > -epsilon && a < epsilon) { return false; } // The vector is parallel to the plane (the intersection is at infinity)

		double f = 1 / a;
		Vector3d s = Vector3d.subtract(o, v1);
		double u = f * Vector3d.dot(s, q);

		if (u < 0.0) return false;// The intersection is outside of the triangle
