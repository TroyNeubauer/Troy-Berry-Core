...
public class ResolutionUtil {
	...
	/**
	 * Creates a new JFrame with the size of the primary monitor * the size
	 * parameter.<br>
	 * IE 0.75 would create a window that is 3/4's the size of the primary
	 * monitor
	 * 
	 * @param size
	 *            The value to scale the size down by 0.001 - 1.0
	...
	public static JFrame createWindowOnPrimaryMonitor(double size) {
		...
		DisplayMode device = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
				.getDisplayMode();
		Resolution resolution = new Resolution((int) (device.getWidth() * size), (int) (device.getHeight() * size),
				(size == 1.0));
	 ...
	 * IE 0.75 would return a resolution that is 3/4's the size of the primary
	 * monitor
	 * 
	 * @param size
	 *            The value to scale the size down by 0.01 - 1.0
	 * @return
	 */
	 ...
	 * 
	 * @param frame
	 *            The frame to extract the resolution from
	...
	public static Resolution getResolution(JFrame frame) {
		return new Resolution(frame.getWidth(), frame.getHeight(),
				frame.getWidth() == GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
						.getDisplayMode().getWidth()
						&& frame.getHeight() == GraphicsEnvironment.getLocalGraphicsEnvironment()
	...
	public static AspectRatio getAspectRatio(JFrame frame) {
		return new Resolution(frame.getWidth(), frame.getHeight(),
				frame.getWidth() == GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
						.getDisplayMode().getWidth()
						&& frame.getHeight() == GraphicsEnvironment.getLocalGraphicsEnvironment()
								.getDefaultScreenDevice().getDisplayMode().getHeight()).aspectRatio;
	}
